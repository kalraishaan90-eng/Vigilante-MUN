...
function generateInfinityPath(cx, cy, w, h) {
  const hw = w / 2;
  const hh = h / 2;
  return `M ${cx} ${cy} C ${cx + hw * 0.5} ${cy - hh}, ${cx + hw} ${cy - hh}, ${cx + hw} ${cy} C ${cx + hw} ${cy + hh}, ${cx + hw * 0.5} ${cy + hh}, ${cx} ${cy} C ${cx - hw * 0.5} ${cy + hh}, ${cx - hw} ${cy + hh}, ${cx - hw} ${cy} C ${cx - hw} ${cy - hh}, ${cx - hw * 0.5} ${cy - hh}, ${cx}`;
}
...
export default function OrbitImages({
  ...
  const items = customItems || images.map((src, index) => {
    const isObject = typeof src === 'object' && src !== null;
    const imgSrc = isObject ? src.image || src.src || src.photo : src;
    const imgAlt = isObject ? src.text || src.name || `${altPrefix} ${index + 1}` : `${altPrefix} ${index + 1}`;
    const nameStr = isObject ? src.name || src.text : null;

    return (
      <div key={imgSrc || index} className="orbit-item-card" title={nameStr || imgAlt}>
        <img
          src={imgSrc}
          alt={imgAlt}
          draggable={false}
        ...
        />
        {nameStr && <div className="orbit-item-badge">{nameStr}</div>}
      </div>
    );
  });
